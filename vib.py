...
while True:
    start = time.time(); ts = str(int(time.time())); dt = str(datetime.datetime.utcnow()); date_utc = datetime.datetime.utcnow().strftime("%Y-%m-%d")
...
#    print(time.time()-next_start)
    ...
    varx = dt + "," + var0 + "," + var1 + "," + str(interval) + "," + str(rest_zeit)
    varx_enc = base64.b64encode(varx.encode())
#    print(varx_enc)
    text_file = open(path+date_utc+".csv", "a")
    text_file.write(varx + "\n")
    text_file.close()
