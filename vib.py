...
while True:
    start = time.time(); ts = str(int(time.time())); dt = datetime.datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S"); date_utc = datetime.datetime.utcnow().strftime("%Y-%m-%d")
...
#    print(time.time()-next_start)
    ...
    text_file = open("~/vib/"+date_utc+".csv", "a")
